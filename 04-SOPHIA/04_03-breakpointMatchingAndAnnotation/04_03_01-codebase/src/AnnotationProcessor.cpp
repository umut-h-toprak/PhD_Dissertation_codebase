...
AnnotationProcessor::AnnotationProcessor(const std::string& tumorResultsIn, std::vector<std::vector<MrefEntryAnno>> &mref, int defaultReadLengthTumorIn, bool controlCheckMode, int germlineDbLimit) :
	...
	while (std::getline(*tumorGzHandle, line)) {
		...
		Breakpoint tmpBp { line, false };
		auto chrIndex = ChrConverter::indexConverter[tmpBp.getChrIndex()];
		std::vector<std::string> tmpOverhangs{};
		if (chrIndex < 0) {
			continue;
		}
		...
		if (hasOverhang) {
			std::string overhang { };
			std::string tmpOverhang { };
			for (auto cit = line.crbegin(); cit != line.crend(); ++cit) {
				if (*cit == '\t') {
					break;
				}else {
					overhang.push_back(*cit);
				}
			}
...
AnnotationProcessor::AnnotationProcessor(const std::string& tumorResultsIn, std::vector<std::vector<MrefEntryAnno>> &mref, const std::string& controlResultsIn, int defaultReadLengthTumorIn, int defaultReadLengthControlIn, int germlineDbLimit, int lowQualControlIn, bool pathogenInControlIn) :
	...
	while (std::getline(*controlGzHandle, line)) {
		...
		Breakpoint tmpBpPre { line, false };
...
void AnnotationProcessor::searchSa(int chrIndex, int dbIndex, const SuppAlignmentAnno& sa, bool doubleSupportSa, std::vector<std::vector<MrefEntryAnno>> &mref) {
	...
	auto preProtectedSa=(sa.getSupport()>8 && sa.getMateSupport()>4) || (sa.getSupport()>4 && sa.getMateSupport()>9);
	if(preProtectedSa){
			if(sa.getSupport()>200||sa.getMateSupport()>200){
				if(sa.getSecondarySupport()<30){
					preProtectedSa=false;
				}
			}
			if(sa.getSupport()>500||sa.getMateSupport()>500){
				if(sa.getSecondarySupport()<50){
					preProtectedSa=false;
				}
			}
	}
	if(preProtectedSa){
		if(sa.isFuzzy()){
			if(sa.getSecondarySupport()==0||tumorResults[chrIndex][dbIndex].distanceToSupp(sa)<10000){
				preProtectedSa=false;
			}
		}
	}
	if(preProtectedSa){
		if (tumorResults[chrIndex][dbIndex].getMrefHits().getNumConsevativeHits() == -1) {
			auto mrefInfo = searchMrefHitsNew(tumorResults[chrIndex][dbIndex], SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD, mref);
			if(mrefInfo.getNumConsevativeHits()>SvEvent::GERMLINEDBLIMIT){
				preProtectedSa=false;
			}
			auto germlineInfo = searchGermlineHitsNew(tumorResults[chrIndex][dbIndex], SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD);
			tumorResults[chrIndex][dbIndex].setGermlineInfo(germlineInfo);
			tumorResults[chrIndex][dbIndex].setMrefHits(mrefInfo);
		}else{
			if(tumorResults[chrIndex][dbIndex].getMrefHits().getNumConsevativeHits()>SvEvent::GERMLINEDBLIMIT){
				preProtectedSa=false;
			}
		}
	}
	if(sa.getChrIndex()==36){
		auto score=createUnknownMatchSv(tumorResults[chrIndex][dbIndex], sa, mref, doubleSupportSa);
		if(preProtectedSa&&score<3){
			addProtectedCase(sa,mref,tumorResults[chrIndex][dbIndex],nullptr,false);
		}
//		std::cerr<<filteredResults.back().printMatch(overhangs)<<"\n";
		return;
	}
	...
	if (!tumorResults[saChrIndex].empty()) {
		...
		if (itStart != tumorResults[saChrIndex].begin() && !itStart->closeToSupp(sa, fuzziness)) {
			...
			if (!itStart->closeToSupp(sa, fuzziness)) {
				auto score = 5;
				if (createUnknownMatchSvPreCheck(sa, doubleSupportSa)) {
					score=createUnknownMatchSv(tumorResults[chrIndex][dbIndex], sa, mref, doubleSupportSa);
				}
				if(preProtectedSa&&score<3){
					addProtectedCase(sa,mref,tumorResults[chrIndex][dbIndex],nullptr,false);
				}
...
//	auto toCheck=tumorResults[chrIndex][dbIndex].getChrIndex()==1 &&	tumorResults[chrIndex][dbIndex].getPos()==102271706;
	if (dbHits.empty()) {
		auto score=5;
		if (createUnknownMatchSvPreCheck(sa, doubleSupportSa)) {
			score=createUnknownMatchSv(tumorResults[chrIndex][dbIndex], sa, mref, doubleSupportSa);
		...
		for (auto &res : dbHits) {
			auto currentMatch = false;
			std::pair<int,bool>bestScore {-2000,false};
			SvEvent* bestFilteredSvPointer=nullptr;
			std::vector<SvEvent*> lowQualSvPointers {};
			std::vector<bool> lowQualSvReverseStatuses{};
			for (const auto &saMatch : res.second->getSuppAlignments()) {
				if (tumorResults[chrIndex][dbIndex].closeToSupp(saMatch, fuzziness)) {
					if (createDoubleMatchSvPreCheck(saMatch)) {
						auto score = createDoubleMatchSv(tumorResults[chrIndex][dbIndex], *res.second, sa, saMatch, true, mref,preProtectedSa);
//						if(toCheck){
//							std::cerr<<score.first<<"\n";
//							std::cerr<<score.second<<"\n\n";
//						}
						if(preProtectedSa){
							if(score.first>0){
								if(score.first > bestScore.first && score.first!=1000){
									bestScore=score;
									bestFilteredSvPointer=&filteredResults.back();
								}
							}else{
								lowQualSvPointers.push_back(&filteredResults.back());
								lowQualSvReverseStatuses.push_back(score.second);
							}
						}
						createdMatch = true;
						currentMatch = true;
			...
			if(preProtectedSa){
				if(currentMatch){
					if(bestScore.first<3){
						if(bestFilteredSvPointer){
							addProtectedCase(sa,mref,tumorResults[chrIndex][dbIndex],bestFilteredSvPointer,bestScore.second);
						}else if(lowQualSvPointers.size()>0){
							addProtectedCase(sa,mref,tumorResults[chrIndex][dbIndex],lowQualSvPointers[0],lowQualSvReverseStatuses[0]);
						}
					}
				}
			}
		...
		if(!createdMatch){
			...
			for (const auto &saMatch : res.second->getSuppAlignments()) {
				if (tumorResults[chrIndex][dbIndex].closeToSupp(saMatch, fuzziness * 3)) {
					if (createDoubleMatchSvPreCheck(saMatch)) {
						auto score = createDoubleMatchSv(tumorResults[chrIndex][dbIndex], *res.second, sa, saMatch, true, mref,preProtectedSa);
						if(preProtectedSa&&score.first<3){
							addProtectedCase(sa,mref,tumorResults[chrIndex][dbIndex],nullptr,score.second);
						}
						return;
					}
				}
			}
			auto score = createUnmatchedSaSv(tumorResults[chrIndex][dbIndex], *res.second, sa, mref,preProtectedSa);
			if(preProtectedSa&&score<3){
				addProtectedCase(sa,mref,tumorResults[chrIndex][dbIndex],nullptr,false);
			}
		}
...
std::pair<int,bool> AnnotationProcessor::createDoubleMatchSv(BreakpointReduced& sourceBp, BreakpointReduced& targetBp, const SuppAlignmentAnno& sa, const SuppAlignmentAnno& saMatch, bool checkOrder, std::vector<std::vector<MrefEntryAnno>>& mref, bool preProtectedSa) {
	if (checkOrder) {
		...
		if (targetBp.fullSmaller(sourceBp)) {
			return createDoubleMatchSv(targetBp, sourceBp, saMatch, sa, false, mref, preProtectedSa);
		}
	}
	...
	if(preProtectedSa){
		if(checkOrder){
			filteredResults.back().setPreProtectedSa1(true);
		}else{
			filteredResults.back().setPreProtectedSa2(true);
		}
	}
	return {checkSvQuality(preProtectedSa),checkOrder};
}
...
int AnnotationProcessor::createUnmatchedSaSv(BreakpointReduced& sourceBp, BreakpointReduced& targetBp, const SuppAlignmentAnno& sa, std::vector<std::vector<MrefEntryAnno>>& mref, bool preProtectedSa) {
	if (sourceBp.getMrefHits().getNumConsevativeHits() == -1) {
		auto germlineInfo = searchGermlineHitsNew(sourceBp, SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD);
		sourceBp.setGermlineInfo(germlineInfo);
		auto mrefInfo = searchMrefHitsNew(sourceBp, SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD, mref);
	...
	return checkSvQuality(preProtectedSa);
}
...
int AnnotationProcessor::createUnknownMatchSv(BreakpointReduced& sourceBp, const SuppAlignmentAnno& sa, std::vector<std::vector<MrefEntryAnno>>& mref, bool preProtectedSa) {
	auto germlineInfo = searchGermlineHitsNew(sourceBp, SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD);
	sourceBp.setGermlineInfo(germlineInfo);
	auto mrefInfo = searchMrefHitsNew(sourceBp, SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD, mref);
	sourceBp.setMrefHits(mrefInfo);
	BreakpointReduced dummyBp { sa, sourceBp, false };
	auto dummyGermline = searchGermlineHitsNew(dummyBp, SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD);
	auto dummyMref = searchMrefHitsNew(dummyBp, SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD, mref);
	if (sa.isFuzzy() && sa.getExtendedPos() - sa.getPos() > 3 * SvEvent::GERMLINEOFFSETTHRESHOLD) {
		BreakpointReduced dummyBp2 { sa, sourceBp, true };
		auto dummyGermline2 = searchGermlineHitsNew(dummyBp2, SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD);
		auto dummyMref2 = searchMrefHitsNew(dummyBp2, SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD, mref);
...
int AnnotationProcessor::checkSvQuality(bool preProtectedSa) {
	auto key = filteredResults.back().getKey(preProtectedSa);
	auto foundMatch=false;
	if (!key.empty()) {
		foundMatch=filteredResultKeys.count(key) != 0;
		if (!foundMatch) {
			auto score=filteredResults.back().getCombinedScore();
...
void AnnotationProcessor::addProtectedCase(
		const  SuppAlignmentAnno& sa,
		std::vector<std::vector<MrefEntryAnno>>& mref,
		const BreakpointReduced& bp,
		SvEvent* svPtr,
		bool switched
		){
//	auto toCheck=bp.getChrIndex()==1 &&	bp.getPos()==102271706;
	if(!svPtr){
		svPtr=&filteredResults.back();
	}
	if(!switched){
		if(!svPtr->isMrefHits1Conservative()){
			if(svPtr->getMrefHits1()>SvEvent::GERMLINEDBLIMIT){
				return;
			}
		}
	}else{
		if(!svPtr->isMrefHits2Conservative()){
			if(svPtr->getMrefHits2()>SvEvent::GERMLINEDBLIMIT){
				return;
			}
		}
	}

	BreakpointReduced dummyBp { sa, bp, false };
	auto dummyMref = searchMrefHitsNew(dummyBp, SuppAlignmentAnno::DEFAULTREADLENGTH * 6, SvEvent::GERMLINEOFFSETTHRESHOLD, mref);
	if(dummyMref.getNumHits()>SvEvent::GERMLINEDBLIMIT){
//		if(toCheck){
//			std::cerr<<"c\n";
//		}
		return;
	}
	auto oldKey=svPtr->getKey(false);
//	auto prevScore=svPtr->getEventScore();
//	auto prevSuspicious=svPtr->getSuspicious();
	auto rescueSuccess=svPtr->rescuePreProtectedSv();
	if(!rescueSuccess){
		return;
	}
	if (!oldKey.empty()) {
		auto it =filteredResultKeys.find(oldKey);
		if ( it != filteredResultKeys.end()) {
			filteredResultKeys.erase(it,filteredResultKeys.end());
		}
	}
//	auto key = svPtr->getKey(false);
//	if (!key.empty()) {
//		if (filteredResultKeys.count(key) == 0) {
//			filteredResultKeys.insert(key);
//		}else{
//			svPtr->setEventScore(prevScore);
//			svPtr->setSuspicious(prevSuspicious);
//			filteredResultKeys.insert(oldKey);
//		}
//	}
}
...
MrefMatch AnnotationProcessor::searchMrefHitsNew(const BreakpointReduced& bpIn, int distanceThreshold, int conservativeDistanceThreshold, std::vector<std::vector<MrefEntryAnno> >& mref) {
	...
	if (itStart != mref[convertedChrIndex].begin() &&
			...
			std::prev(itStart)->distanceTo(bpIn) < SvEvent::GERMLINEOFFSETTHRESHOLD) {
		--itStart;
	}
	auto it = itStart;
	...
	return MrefMatch { score, conservativeScore, offset, suppMatches };
}
...
void AnnotationProcessor::printFilteredResults(bool contaminationInControl, int controlPrefilteringLevel) const {
	...
	for (const auto &sv : filteredResults) {
		if (!sv.isToRemove()) {
			if(SvEvent::DEBUGMODE&&sv.getSuspicious()>0){
				continue;
			}
		...
		for (const auto &bp : tumorChromosome) {
			...
			if (bp.testOverhangBasedCandidacy()) {
				...
				if (mrefHits.getNumHits() < SvEvent::GERMLINEDBLIMIT) {
					...
					if (!overhang.empty()) {
						std::cout << bp.printOverhang(germlineClonality, mrefHits.getNumConsevativeHits(), overhang);
					}
				}
			}
		}
