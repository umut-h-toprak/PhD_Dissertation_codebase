...
SuppAlignmentAnno::SuppAlignmentAnno(
		...
		bool properPairErrorProneIn):
				...
				support { supportIn },
				secondarySupport { secondarySupportIn },
				...
				strictFuzzy { false },
				strictFuzzyCandidate { (supportIn + secondarySupportIn) < 3 },
				distant { false },
				suspicious { suspiciousIn },
				semiSuspicious { semiSuspiciousIn },
				properPairErrorProne { properPairErrorProneIn },
				supportingIndices {  }
			{
				if (support + secondarySupport == 0) {
						fuzzy = true;
...
SuppAlignmentAnno::SuppAlignmentAnno(int emittingBpChrIndex, int emittingBpPos, const SuppAlignmentAnno& saAnnoIn) :
				...
				toRemove { true },
				inverted { saAnnoIn.isInverted() },
				fuzzy { saAnnoIn.isDistant() },
				strictFuzzy { saAnnoIn.isDistant() },
...
bool SuppAlignmentAnno::saCloseness(const SuppAlignmentAnno& rhs, int fuzziness) const {
...
bool SuppAlignmentAnno::saClosenessDirectional(const SuppAlignmentAnno& rhs, int fuzziness) const {
	//inverted == rhs.isInverted()  &&   //test
