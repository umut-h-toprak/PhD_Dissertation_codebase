...
void Alignment::continueConstruction() {
	mappingQualityCheck();
	for (auto startPos_cit = samLine.cbegin() + 1 + samChunkPositions[2]; startPos_cit != samLine.cbegin() + samChunkPositions[3]; ++startPos_cit) {
			...
			if (supplementary) {
				...
				for (const auto &cigarChunk : cigarChunks) {
					if (cigarChunk.chunkType == 'S') {
						...
						if (cigarChunk.length > LOWQUALCLIPTHRESHOLD && medianQual < BASEQUALITYTHRESHOLD) {
	...
	for (auto mpos_cit = samLine.cbegin() + 1 + samChunkPositions[6]; mpos_cit != samLine.cbegin() + samChunkPositions[7]; ++mpos_cit) {
...
void Alignment::mappingQualityCheck() {
	if (samLine[1 + samChunkPositions[3]] != '0') { //mapq 0 is treated as a special case, where number of SAs and base qualities will be the sole determinants of read quality
		nullMapq = false;
		...
		case 2: //this checks if the mapq is a single-digit number
			...
			break;
		case 3:
			if (samLine[1 + samChunkPositions[3]] == '1' && ((samLine[2 + samChunkPositions[3]] - '0') < 3)) {
				//this checks if the mapq is a two-digit number, and if the first digit is a "1", that the second digit is less than 3
				readType = 7;
				lowMapq = true;
			}
			break;
...
bool Alignment::isEventCandidate() const {
	if (samLine[samChunkPositions[5] - 1] != 'M') {
		return true;
	} else {
		for (auto cigarString_cit = samLine.cbegin() + 1 + samChunkPositions[4]; cigarString_cit != samLine.cbegin() + samChunkPositions[5] - 1; ++cigarString_cit) {
...
void Alignment::createCigarChunks() {
	...
	for (auto cigarString_cit = samLine.cbegin() + 1 + samChunkPositions[4]; cigarString_cit != samLine.cbegin() + samChunkPositions[5]; ++cigarString_cit) {
		if (std::isdigit(*cigarString_cit)) {
			...
			case 'M':
				...
				break;
			case 'S':
				cigarChunks.emplace_back(*cigarString_cit, encounteredM, cumulativeNucleotideCount + indelAdjustment - leftClipAdjustment, currentNucleotideCount, indelAdjustment - leftClipAdjustment);
				cumulativeNucleotideCount += currentNucleotideCount;
				if (!encounteredM) {
				...
				break;
			case 'H':
				cigarChunks.emplace_back(*cigarString_cit, encounteredM, cumulativeNucleotideCount + indelAdjustment - leftClipAdjustment, currentNucleotideCount);
				break;
			case 'I':
				cigarChunks.emplace_back(*cigarString_cit, encounteredM, cumulativeNucleotideCount + indelAdjustment - leftClipAdjustment, currentNucleotideCount);
				cumulativeNucleotideCount += currentNucleotideCount;
...
void Alignment::assignBreakpointsAndOverhangs() {
	for (const auto &chunk : cigarChunks) {
		...
		case 'S':
			...
			if (chunk.encounteredM) {
				...
				readOverhangCoords.emplace_back(chunk.encounteredM, endPos, chunk.startPosOnRead - chunk.indelAdjustment, chunk.length);
			} else {
				readBreakpoints.push_back(startPos);
				readOverhangCoords.emplace_back(chunk.encounteredM, startPos, chunk.startPosOnRead - chunk.indelAdjustment, chunk.length);
			}
			break;
		case 'H':
			if (chunk.encounteredM) {
		...
		case 'D':
			...
			readBreakpointSizes.push_back(chunk.length);
			readBreakpointsEncounteredM.push_back(chunk.encounteredM);
			readBreakpoints.push_back(startPos + chunk.startPosOnRead + chunk.length);
...
bool Alignment::uniqueSuppCheck() {
	...
	saCend = samLine.cend();
	if (samLine.back() == ';' && samLine[samChunkPositions.back() + 1] == 'S' && samLine[samChunkPositions.back() + 2] == 'A') {
		saCbegin = samLine.cbegin() + samChunkPositions.back() + 6;
		saCend = samLine.cend() - 1;
		hasSa = true;
	} else {
		for (auto i = 10u; i < samChunkPositions.size() - 1; ++i) {
			if (samLine[samChunkPositions[i + 1] - 1] == ';' && samLine[samChunkPositions[i] + 1] == 'S' && samLine[samChunkPositions[i] + 2] == 'A') {
	...
	if (hasSa) {
		...
		for (auto saCit = saCbegin; saCit != saCend; ++saCit) {
			...
			case ';':
				...
				if (!highQualSa && ((sCounts == 1 && lowQualSacounts == 2) || (hCounts == 1 && lowQualSacounts == 2) || (sCounts == 2 && lowQualSacounts == 4))) {
					return false;
				}
				block = 0;
				mapq = 0;
...
double Alignment::overhangMedianQuality(const CigarChunk& cigarChunk) const {
	...
	if (overhangPerBaseQuality.empty()) {
		...
		return getMedian(overhangPerBaseQuality.begin(), overhangPerBaseQuality.end());
	}
}

void Alignment::assessReadType() {
	/* 0 for non-split,
	 * 1 for softSplit,
	 * 2 for hardSplit,
	 * 3 for indel,
...
bool Alignment::assessOutlierMateDistance() {
	switch (distantMate) {
	case -1:
		return false;
	case 1:
	...
	default:
		if (*(samLine.cbegin() + 1 + samChunkPositions[5]) != '=') {
			...
			auto isize = 0;
			for (; isize_cit != samLine.cbegin() + samChunkPositions[8]; ++isize_cit) {
...
void Alignment::setChosenBp(int chosenBpLoc, int alignmentIndex) {
			...
			if (bpType == 'S') {
				for (const auto &overhang : readOverhangCoords) {
					if (overhang.bpPos == chosenBpLoc) {
						overhangStartIndex = 1 + samChunkPositions[8] + overhang.startPosOnRead;
...
std::vector<SuppAlignment> Alignment::generateSuppAlignments(int bpChrIndex, int bpPos) {
	...
	if (hasSa) {
		...
		for (auto i = 0u; i < saBegins.size(); ++i) {
			SuppAlignment saTmp { saBegins[i], saEnds[i], !supplementary, lowMapq, nullMapq, fwdStrand, chosenBp->bpEncounteredM, chosenBp->selfNodeIndex, bpChrIndex, bpPos };
	...
	if (assessOutlierMateDistance()) {
		if (getMateChrIndex() < 1002) {
			...
			MateInfo tmpPairDummy { 0, 0, getMateChrIndex(), getMatePos(), true, invertedMate };
			...
			if (!foundMatch) {
				suppAlignmentsTmp.emplace_back(getMateChrIndex(), getMatePos(), 0, 0, chosenBp->bpEncounteredM, invertedMate, getMatePos() + 1, !supplementary, lowMapq, nullMapq, chosenBp->selfNodeIndex);
			}
		}
...
std::string Alignment::printOverhang() const {
	...
	if (chosenBp->bpEncounteredM) {
		...
		res.append(samLine.substr(chosenBp->overhangStartIndex, chosenBp->overhangLength)).append("|");
	}
	res.append("(").append(strtk::type_to_string<int>(chosenBp->childrenNodes.size())).append(")");
	return res;
}
			...
			if (!overhang.empty()) {
				...
				if (!res.empty()) {
					for (const auto &resInterval : res) {
						maskedIntervalsTotal += resInterval.endIndex - resInterval.startIndex + 1;
					}
				}
				overhang.clear();
			}
