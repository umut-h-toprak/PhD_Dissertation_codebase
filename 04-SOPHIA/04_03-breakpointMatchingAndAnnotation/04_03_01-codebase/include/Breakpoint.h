...
public:
	...
	Breakpoint(const std::string& bpIn, bool ignoreOverhang);
	~Breakpoint() = default;
	...
	static int bpindex;
	static const std::string COLUMNSSTR;
	void addSoftAlignment(std::shared_ptr<Alignment> alignmentIn);
	void addHardAlignment(std::shared_ptr<Alignment> alignmentIn);
	bool finalizeBreakpoint(const std::deque<MateInfo>& discordantAlignmentsPool, const std::deque<MateInfo>& discordantLowQualAlignmentsPool, const std::deque<MateInfo>& discordantAlignmentCandidatesPool);
	void setLeftCoverage(int leftCoverageIn) {
		leftCoverage = leftCoverageIn;
	}
	...
	void setNormalSpans(int normalSpansIn) {
		normalSpans = normalSpansIn;
	}
	...
	bool isCovFinalized() const {
		return covFinalized;
	}
	void setCovFinalized(bool covFinalizedIn) {
		covFinalized = covFinalizedIn;
	}
	template<typename T>
	bool operator<(const T& rhs) const {
		if (chrIndex < rhs.getChrIndex()) return true;
		if (chrIndex > rhs.getChrIndex()) return false;
	...
	bool isMissingInfoBp() const {
		return missingInfoBp;
	}
	...
	void setGermline(bool germlineIn) {
		this->germline = germlineIn;
	}

	void setHitsInMref(int hitsInMref) {
		this->hitsInMref = hitsInMref;
	}

private:
	std::string finalizeOverhangs();
	void printBreakpointReport(const std::string& overhangStr);
	bool matchDetector(const std::shared_ptr<Alignment>& longAlignment, const std::shared_ptr<Alignment>& shortAlignment) const;
	void detectDoubleSupportSupps();
	void collapseSuppRange(std::string &res, const std::vector<SuppAlignment>& vec) const;
	template<typename T>
	void cleanUpVector(std::vector<T>& objectPool);
	void fillMatePool(const std::deque<MateInfo>& discordantAlignmentsPool, const std::deque<MateInfo>& discordantLowQualAlignmentsPool, const std::deque<MateInfo>& discordantAlignmentCandidatesPool);
	void collectMateSupport();
	void compressMatePool(std::vector<MateInfo>& discordantAlignmentsPool);
	void collectMateSupportHelper(SuppAlignment& sa, std::vector<MateInfo>& discordantAlignmentsPool, std::vector<MateInfo>& discordantLowQualAlignmentsPool);
...
template<typename T>
inline void Breakpoint::cleanUpVector(std::vector<T>& objectPool) {
