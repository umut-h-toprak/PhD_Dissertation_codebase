...
void Sdust::saveMaskedRegions(int wStart) {
	...
	for (auto i = static_cast<int>(w.size()) - L - 1; i >= 0; --i) {
		...
		if ((newScore * 10) > SCORETHRESHOLD) {
			...
			if (newScore >= maxScore) {
				P.emplace_hint(cit, PerfectInterval { i + wStart, static_cast<int>(w.size()) + 1 + wStart, newScore });
			}
		}
	}
}

void Sdust::shiftWindow(int t) {
