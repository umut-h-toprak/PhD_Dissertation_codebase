...
void SamSegmentMapper::printBps(int alignmentStart) {
	for (auto &bp : breakpointsCurrent) {
		if (!bp.second.isCovFinalized() && ((bp.first) + 1 < alignmentStart)) {
			auto posDiff = bp.first - minPos;
			if (bp.first != minPos) {
				bp.second.setLeftCoverage(coverageProfiles[bp.first - minPos - 1].getCoverage());
			} else {
				bp.second.setLeftCoverage(0);
			}
			bp.second.setRightCoverage(coverageProfiles[posDiff].getCoverage());
			bp.second.setNormalSpans(coverageProfiles[posDiff].getNormalSpans());
			bp.second.setLowQualSpansSoft(coverageProfiles[posDiff].getLowQualSpansSoft());
			bp.second.setLowQualSpansHard(coverageProfiles[posDiff].getLowQualSpansHard());
			bp.second.setUnpairedBreaksSoft(coverageProfiles[posDiff].getNormalBpsSoft());
			bp.second.setUnpairedBreaksHard(coverageProfiles[posDiff].getNormalBpsHard());
			bp.second.setBreaksShortIndel(coverageProfiles[posDiff].getNormalBpsShortIndel());
			bp.second.setLowQualBreaksSoft(coverageProfiles[posDiff].getLowQualBpsSoft());
			bp.second.setLowQualBreaksHard(coverageProfiles[posDiff].getLowQualBpsHard());
	...
	for (auto bpIt = breakpointsCurrent.begin(); bpIt != breakpointsCurrent.end();) {
		if ((bpIt->first) + DISCORDANTRIGHTRANGE < alignmentStart) {
			if (bpIt->second.finalizeBreakpoint(discordantAlignmentsPool, discordantLowQualAlignmentsPool, discordantAlignmentCandidatesPool)) {
				++printedBps;
			}
	...
	while (!discordantAlignmentsPool.empty() && (discordantAlignmentsPool.front().readStartPos + DISCORDANTLEFTRANGE + DISCORDANTRIGHTRANGE < alignmentStart)) {
		discordantAlignmentsPool.pop_front();
	}
	if (PROPERPARIRCOMPENSATIONMODE) {
		while (!discordantAlignmentCandidatesPool.empty() && (discordantAlignmentCandidatesPool.front().readStartPos + DISCORDANTLEFTRANGE + DISCORDANTRIGHTRANGE < alignmentStart)) {
			discordantAlignmentCandidatesPool.pop_front();
		}
	}
	while (!discordantLowQualAlignmentsPool.empty() && (discordantLowQualAlignmentsPool.front().readStartPos + DISCORDANTLEFTRANGE + DISCORDANTRIGHTRANGE < alignmentStart)) {
		discordantLowQualAlignmentsPool.pop_front();
	}
}

void SamSegmentMapper::incrementCoverages(const Alignment& alignment) {
	if (minPos == -1) {
	...
	case 3:
		...
		if (PROPERPARIRCOMPENSATIONMODE) {
			discordantAlignmentCandidatesPool.emplace_back(alignment.getStartPos(), alignment.getEndPos(), -1, -1, -1, false);
		}
		break;
	case 1:
		for (auto i = alignment.getStartPos(); i < alignment.getEndPos(); ++i) {
	...
	case 4:
		...
		if (alignment.getMateChrIndex() < 1002 && !(alignment.getMateChrIndex() == 2 && (alignment.getMatePos() / 10000 == 3314))) {
			if (PROPERPARIRCOMPENSATIONMODE) {
				discordantAlignmentCandidatesPool.emplace_back(alignment.getStartPos(), alignment.getEndPos(), alignment.getMateChrIndex(), alignment.getMatePos(), 2, alignment.isInvertedMate());

			}
			if (!alignment.isNullMapq()) {
				discordantAlignmentsPool.emplace_back(alignment.getStartPos(), alignment.getEndPos(), alignment.getMateChrIndex(), alignment.getMatePos(), 2, alignment.isInvertedMate());
			} else {
				discordantLowQualAlignmentsPool.emplace_back(alignment.getStartPos(), alignment.getEndPos(), alignment.getMateChrIndex(), alignment.getMatePos(), 2, alignment.isInvertedMate());
			}
		}
		break;
	case 2:
	...
	case 5:
		...
		if (!alignment.isSupplementary() && alignment.getMateChrIndex() < 1002 && alignment.isDistantMate()) {
			if (!(alignment.getMateChrIndex() == 2 && (alignment.getMatePos() / 10000 == 3314))) {
				discordantLowQualAlignmentsPool.emplace_back(alignment.getStartPos(), alignment.getEndPos(), alignment.getMateChrIndex(), alignment.getMatePos(), 2, alignment.isInvertedMate(), alignment.getReadBreakpoints());
			}
		}
		break;
	default:
	...
	case 1:
		for (auto j = 0u; j < alignment.getReadBreakpoints().size(); ++j) {
			...
			case 'D':
				coverageProfiles[bpPos - minPos].incrementNormalBpsShortIndel();
				for (auto k = 0; k != alignment.getReadBreakpointsSizes()[j]; ++k) {
	...
	case 2:
		if (!(alignment.isLowMapq() || alignment.isNullMapq())) {
			...
			for (auto j = 0u; j < alignment.getReadBreakpoints().size(); ++j) {
				...
				case 'S':
					if (bpPos == alignment.getStartPos()) {
						coverageProfiles[bpPos - minPos].decrementLowQualSpansHard();
					}
				...
				case 'D':
					coverageProfiles[bpPos - minPos].incrementLowQualBpsHard();
					for (auto k = 0; k != alignment.getReadBreakpointsSizes()[j]; ++k) {
						coverageProfiles[bpPos - minPos + k].decrementLowQualSpansHard();
					}
					break;
				default:
					break;
	...
	case 5:
		for (auto j = 0u; j < alignment.getReadBreakpoints().size(); ++j) {
			...
			case 'H':
				if (bpPos == alignment.getStartPos()) {
					coverageProfiles[bpPos - minPos].decrementLowQualSpansSoft();
				}
			...
			case 'D':
				coverageProfiles[bpPos - minPos].incrementLowQualBpsSoft();
				for (auto k = 0; k != alignment.getReadBreakpointsSizes()[j]; ++k) {
					coverageProfiles[bpPos - minPos + k].decrementLowQualSpansSoft();
				}
				break;
			default:
				break;
