...
$columnsToSelect = $_POST['columnsToSelect'];
$keyColumn = $_POST['keyColumn'];
$keysToSearchRaw= $_POST['keysToSearchRaw'];
...
if ($keysToSearchRaw=="*"){
    $sql = "SELECT $columnsToSelect FROM ".str_replace('-','$',$cohortName)."_"."$suffixName";
    $statement=$db->prepare($sql);
    $statement->execute();
}else{
    $sql = "SELECT $columnsToSelect FROM ".str_replace('-','$',$cohortName)."_"."$suffixName WHERE $keyColumn IN ($placeholder)";
    $statement=$db->prepare($sql);
